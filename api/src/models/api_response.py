...
class APIMetadata(BaseModel):
    ...
    timestamp: str = Field(
        default_factory=lambda: datetime.now(timezone_sao_paulo).isoformat() + "Z"
    )
...
class PredictionResult(BaseModel):
    ...
    survival_probability: float = Field(
        ..., ge=0.0, le=1.0, description="Probabilidade de sobrevivência (0.0 a 1.0)"
    )
    prediction: str = Field(
        ..., description="Predição categórica (survived/not_survived)"
    )
    confidence_level: str = Field(..., description="Nível de confiança da predição")

    @classmethod
    def from_probability(
        cls, passenger_id: str, probability: float
    ) -> "PredictionResult":
